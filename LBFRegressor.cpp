...
void LBFRegressor::Train(vector<Mat_<uchar> >& images,
                         ...
                         vector<BoundingBox> & bounding_boxs, int posLenth){
    ...
    for(int i = 0;i < images.size();i++){
        for(int j = 0;j < global_params.initial_num;j++){
			if (ground_truth_faces[i]==-1)
			{
				int tmp_idx;
				do{
					tmp_idx = random_generator.uniform(0, (int)images.size()-1);
				}while(tmp_idx == i);
				BoundingBox newbox;
				getRandomBox(images[i],bounding_boxs[tmp_idx], newbox);
				Mat_<float> temp1 = ProjectShape(ground_truth_shapes[tmp_idx], bounding_boxs[tmp_idx]);
				Mat_<float> temp2 = ReProjectShape(temp1, newbox);
				// 1. Select ground truth shapes of other images as initial shapes
				augmented_images.push_back(i);
				augmented_ground_truth_shapes.push_back(temp2);
				augmented_ground_truth_faces.push_back(ground_truth_faces[i]);
				augmented_bounding_boxs.push_back(newbox);
				...
				for (int k = 0; k < mean_shape_.rows; k++) {
					circle(ttttttt, Point(current_shapes[current_shapes.size()-1](k, 0), current_shapes[current_shapes.size() - 1](k, 1)), 3, Scalar(255));
				}
				rectangle(ttttttt, Point(newbox.start_x, newbox.start_y),
					Point(newbox.start_x + newbox.width, newbox.start_y + newbox.height), Scalar(255));
			...
			else
			{
				...
				do {
					...
					index = random_generator.uniform(0, (int)images.size()-1);
				} while (index == i);

				// 2. Project current shape to bounding box of ground truth shapes
				Mat_<float> temp2 = ProjectShape(ground_truth_shapes[index], bounding_boxs[index]);
				temp2 = ReProjectShape(temp2, bounding_boxs[i]);
				current_shapes.push_back(temp2);
				...
				for(int k=0;k<temp2.rows;k++){
					circle(ttttttt,Point(temp2(k,0), temp2(k,1)),3,Scalar(255));
				}
				rectangle(ttttttt,Point(bounding_boxs[i].start_x, bounding_boxs[i].start_y),
					Point(bounding_boxs[i].start_x+ bounding_boxs[i].width, bounding_boxs[i].start_y+ bounding_boxs[i].height),Scalar(255));
				imshow("test3",ttttttt);
				waitKey(0);*/
			}
        }
    ...
    for (int stage = 0; stage < global_params.max_numstage; stage++){
        ...
        RandomForest_[stage].Train(images, augmented_images, augmented_ground_truth_shapes, 
			augmented_ground_truth_faces,current_shapes, current_fi,current_weight,augmented_bounding_boxs,
			mean_shape_, shapes_residual_, stage, RandomForest_,Models_,posLenth);
...
void  LBFRegressor::WriteRegressor(ofstream& fout){
    ...
    ofstream fout_reg;
	locale::global(locale(""));
    fout_reg.open(dataPath.c_str(),ios::binary);
	locale::global(locale("C"));
    for (int i=0; i < global_params.max_numstage; i++ ){
...
void LBFRegressor::ReadRegressor(ifstream& fin){
    ...
    ifstream fin_reg;
	locale::global(locale(""));
    fin_reg.open(dataPath.c_str(),ios::binary);
	locale::global(locale("C"));
